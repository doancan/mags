...
- **PRD parser h2 heading support**: Parser now accepts both `## M1:` and `### M1:` module headers, fixing compatibility with PRDs generated by MAGS 0.1.0
- **Orchestrator path resolution**: All orchestrator tools now resolve relative paths against `MAGS_PROJECT_ROOT` instead of the server's working directory — `docs/product/prd.md` works correctly
- **ADR validation**: Frontmatter `status` field accepted as alternative to heading-based status in ADR checks
- **Native module auto-rebuild**: Added `check-native` script with `pretest`/`prebuild` hooks that automatically detect and rebuild `better-sqlite3` when Node.js version changes
- **Version consistency**: Plugin manifest and marketplace metadata now updated during releases
- **Doc tools test count**: Corrected expected tool count to include `mags_update_metadata`
